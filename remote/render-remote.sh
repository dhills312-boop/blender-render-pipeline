...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <pod-ssh-host> [blend-file-relative-to-project] [render-script] [config-file] [--terminate-after]"
  exit 1
fi
...
CONFIG_FILE="${4:-render_config.json}"
SSH_OPTS="${SSH_OPTS:-}"
TERMINATE_AFTER="false"
if [[ "${4:-}" == "--terminate-after" || "${5:-}" == "--terminate-after" ]]; then
...
REMOTE_CONFIG="/workspace/render-scripts/${CONFIG_FILE}"
